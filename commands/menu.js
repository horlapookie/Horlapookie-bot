    ...
    try {
      ...
      const categories = {
       ...
       commands: []
        ...
        media: {
          name: '🎵 Music & Media',
          keywords: ['play', 'lyrics', 'yt', 'tiktok', 'tik', 'video', 'vid', 'ytv', 'spotifylist', 'spotifysearch', 'splaylist', 'song', 'musicdoc', 'ytmp3doc', 'audiodoc', 'mp3doc', 'shazam'],
          commands: []
        },
        downloaders: {
          name: '📥 Downloaders',
        ...
        self: {
          ...
          keywords: ['crash', 'vv2', 'block', 'unblock', 'fullpp', 'updatepp', 'ppfull'],
          commands: []
        },
        screenshots: {
          name: '📸 Screenshots',
      ...
      // Build menu from dynamically loaded commands with monospace styling
      let menuText = `\`\`\`
╭─────「 🤖 HORLA POOKIE Bot 」─────╮
│  📅 Time: ${timeString}
...
`;
      ...
      // Add each category that has commands with monospace styling
...
│  🛡️  Availability: 24/7
│  📈 Performance: ${memPercent}% Memory
│  ⏱️  Runtime: ${formattedUptime}
│  🔋 System Load: ${cpuPercent}%
│  📱 Session: Active & Secure
│  🌐 Web Interface: Available at bot dashboard
╰─────────────────────────────╯
\`\`\`

*© ${creatorInfo.name} - HORLA POOKIE Bot v2.0*
*Serving: ${userName} (+${userNumber})*
*Platform: ${process.platform} | Node: ${process.version}*`;
      ...
      try {
        ...
        const randomImage = allImages[Math.floor(Math.random() * allImages.length)] || 'https://i.imgur.com/X8tgH2v.png';

        // First send the menu with image
        ...
        if (mediaUrls.menuAudio) {
          setTimeout(async () => {
            try {
              await sock.sendMessage(from, {
                audio: { url: mediaUrls.menuAudio },
                mimetype: 'audio/mpeg',
                ptt: false,
                ...channelInfo
              }, { quoted: msg });
              console.log(`[INFO] Menu audio sent successfully to: ${from}`);
            } catch (audioError) {
              console.warn(`[WARN] Failed to send menu audio to ${from}: ${audioError.message}`);
            }
