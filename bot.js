...
// Setup auth state from SESSION-ID file
async function setupAuthState() {
  if (!fs.existsSync(SESSION_ID_FILE)) {
    console.log(color('[ERROR] SESSION-ID file not found!', 'red'));
    console.log(color('[INFO] Please create a SESSION-ID file with valid session data.', 'yellow'));
    process.exit(1);
  }

  const sessionData = fs.readFileSync(SESSION_ID_FILE, 'utf8').trim();
  if (!sessionData) {
...
if (fs.existsSync(selfCommandsDir)) {
  ...
  for (const file of selfCommandFiles) {
    try {
      const imported = await import(`file://${path.join(selfCommandsDir, file)}`);
